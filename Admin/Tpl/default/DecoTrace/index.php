...
<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D6DDD6" align="center">
  <tr>
   ...
   <a href="__URL__/add/" class="coolbg" style="display: block;margin:0 auto;width:100px;"> 添加家装追踪用户 </a>
   </td>
  </tr>
...
<div id="_mainsearch">
  ...
  <tbody><tr> 
    <td height="19" colspan="10" background="__PUBLIC__/admin/images/tbg.gif" bgcolor="#E7E7E7">
        ...
        <tbody><tr> 
          <td width="24%" style="padding-left:10px;"><strong>家装追踪用户列表</strong></td>
  ...
  <volist name="traceUsers" id="item">
    ...
    $uid = "'".$item['id']."'";
    if($item['status'] > 0){ 
    	echo '<a href="javascript:void(0)" style="color:green;" onclick="changeUserStatus(0, '.$uid.')">[正常]</a>';
    }else{
    	echo '<a href="javascript:void(0)" style="color:red;" onclick="changeUserStatus(1, '.$uid.')">[停用]</a>';
    }
    ?>
    </td>
    <td>
        <a style="text-decoration: underline;" href="__URL__/edit/cid/{$item.id}" title="修改密码">修改密码</a>
        <a style="text-decoration: underline;" href="__URL__/homeDecoList/uid/{$item.id}" title="家装列表">家装列表</a>    
    </td>
...
function changeUserStatus(s, id){
	var d = "确定激活该用户？";
	if(s == 0){
		d = "确定停用该用户？";
	}
